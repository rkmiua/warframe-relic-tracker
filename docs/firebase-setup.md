...
所要 10 分ほど。設定しなくても共有コード（seed）は使えるので、急ぐ必要はない。
...
2. 中身を全部消して、リポジトリの [`firestore.rules`](../firestore.rules) の内容を貼り付ける
3. 「**公開**」
...
2. 下にスクロールして「**マイアプリ**」
3. **ウェブ**のアイコン（`</>`）を押す
4. アプリのニックネーム: `RelicVault` など
5. 「Firebase Hosting も設定する」は**チェックしない**（GitHub Pages を使うため）
6. 「アプリを登録」
7. `firebaseConfig = { ... }` が表示される。**この 4 つを控える**
   - `apiKey`
   - `authDomain`
   - `projectId`
   - `appId`
