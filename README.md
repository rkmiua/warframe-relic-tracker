...
- **未所持パーツあり** — 自分にまだ足りていない報酬が入っているレリック。
  ひとりで回すときはこれだけでよい
- **全員未所持** — 自分も分隊のみんなも 1 個も持っていない報酬が入っているレリック。
  誰が引いても無駄にならないので、みんなで回すならここから選ぶ
- **誰かが未所持** — 1 人でもまだ足りていない報酬が入っているレリック。こちらの方が広く残る

下の 2 つは見比べる相手が要るので、ルームに入っているときだけ出る。
絞り込んでいる間は、各行の右側もその基準の数（「全員 2」「誰か 4」）に変わる。

Forma は数に入れない。いくつ持っていても足りることがない消耗品で、
数えると全レリックが該当してしまうため。同じ理由で、持ち具合も記録しない
（レリックの報酬としては出るが、記録ボタンは付かず、パーツ検索にも出ない）。
...
新しい Prime が実装されたときや、禁庫（Vault）の出入りがあったときの手順。

```sh
python3 Tools/generate_data.py     # 最新を取ってきて JSON を作り直す
cd web && npm run deploy           # ビルドして配信
```

実行すると前回からの差分が出る。

```
  パーツ番号: 既存 584 件を引き継ぎ、新規 4 件を追加

前回からの変化:
  + レリック 3 件: Axi A23, Lith Q4, Neo V13
  + セット 1 件: Xaku Prime
```

増減がなくても、Vaulted 状態や確率だけが変わっていることはあるので、
そのまま配信してよい。

### パーツ番号を振り直してはいけない

パーツ番号は、みんなが記録した内容のビット位置そのもの。
番号がずれると、**全員の記録が別のパーツのものとして読まれてしまう**。

そのためスクリプトは、既存の `web/public/warframe_data.json` から番号を引き継ぎ、
初めて見るパーツだけを末尾に足す。引き継ぎ元が見つからないときは、
書き換えずに中止する。
...
これが出たら、出力先のパスか、ファイルを消していないかを確かめる。
本当に作り直してよいのは、記録を持っている人が誰もいないときだけで、
そのときは `--first-run` を付ける。
